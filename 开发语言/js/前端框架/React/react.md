...
* 组件定义
  ...
  ```
* 组件传值(props)
  - 组件传值：传递任何 JavaScript 的值，包括对象、数组、函数、甚至是 JSX
...
* 组件内部状态管理（useState）
  ...
  + 组件内常见用法见[ToDoList](./demo.js)
    ...
    - 获取dom: ref
* 组件内的数据获取（useEffect）
  - vue组件中有生命周期钩子函数可以做一些事件触发
  - useEffect 是 React 中的一个核心 Hook，用于在函数组件中处理副作用操作（如数据请求、订阅、DOM 修改等）。它的作用是替代类组件中的生命周期方法（如 componentDidMount、componentDidUpdate、componentWillUnmount）
  + 基本用法
    ```js
        useEffect(() => {
            // 副作用操作（如 API 请求、事件监听）
...
* 组件通讯（useContext）
  ...
  - 跨组件通讯 useContext，类似 vue3 中 provide、inject，解决组件逐级透传的问题
    ...
    ```
* 组件内缓存计算结果（useMemo）
  - 避免在每次组件渲染时重复执行复杂的计算逻辑，从而优化性能。它类似于 Vue 中的 computed 属性，但需要显式指定依赖项
  ```js
