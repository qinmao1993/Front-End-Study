...
* Poetry 现代 Python 包管理的标杆（≈ npm/yarn 的体验），Poetry 统一了以下所有事项：
  - 项目元数据：pyproject.toml（取代 setup.py/setup.cfg）
  - 依赖管理与锁定：poetry.lock
  - 虚拟环境：自动创建和管理
  - 打包发布：内置 poetry build 和 poetry publish
  + 基本用法
    ```bash
        pip install poetry
        poetry new myproject      # 创建新项目
        cd myproject
        poetry add requests       # 添加依赖，自动更新 pyproject.toml 和 lock 文件
        poetry install            # 安装所有依赖（别人 clone 项目后，只需这个）
        poetry shell              # 进入虚拟环境
        poetry run python app.py  # 不用激活环境，直接运行
    ```

* uv（纯python项目推荐）
  > 目前最强的趋势是 uv（由 Astral 团队用 Rust 开发）。它集成了：
  - Python 版本管理（取代 pyenv）
  - 虚拟环境管理
  - 极快的依赖解析与安装（比 pip/pip-tools 快 10-100 倍）
  - 锁定文件 uv.lock（跨平台确定性构建）
  - 项目管理命令 uv init、uv add、uv run
  - 构建与发布 PyPI 包
  ```bash
    # 安装 uv
...
* Anaconda|miniconda（ai数据科学）
  ...
  - 管python包+非 Python 的依赖（如 OpenCV、CUDA、GCC）+环境+解释器
  - [Conda](./conda.md)