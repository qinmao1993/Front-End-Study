# GitLab
GitLab 是一个基于 Git 的一站式 DevSecOps 平台。它将从项目规划、代码托管、安全扫描，到持续集成与部署（CI/CD）、监控等整个软件开发生命周期的所有环节,都整合到了单一应用中

## GitLab 核心功能
* 代码托管与协作
  - Git 仓库管理：提供强大的版本控制功能，支持代码托管、分支管理、历史追溯
  - 代码评审：通过合并请求（Merge Request, MR） 进行协作，支持行内评论、多人审批和自动化质量门禁，确保代码质量。
* 持续集成与部署 (CI/CD)
  - 流水线即代码：通过 .gitlab-ci.yml 文件定义自动化流程，从构建、测试到部署全自动。
  - 灵活的 Runner：支持多种执行环境和标签，可灵活调度流水线任务。
  - Auto DevOps：提供开箱即用的CI/CD最佳实践模板，自动完成环境配置，极大简化上手流程。
  - 高级流水线：支持父子流水线、多项目流水线等复杂架构，满足微服务或复杂系统的需求。
* 项目管理与规划
  - 需求管理：通过 Issue、Epic（史诗）、里程碑等工具，支持 Scrum 和 Kanban 两种主流敏捷开发模式。
  - 看板与可视化：提供可视化的任务看板，清晰展示工作流状态。
  - 价值流分析：量化分析研发全流程耗时，帮助管理者精准定位瓶颈。
* 安全与合规 (DevSecOps)
  - 安全扫描左移：将 SAST、DAST、依赖扫描等安全测试内置到 CI/CD 流水线中，在代码合并前发现并修复漏洞。
  - 合规管理：提供合规框架、策略管理和详细的审计事件，满足 GDPR、SOX 等全球合规标准。
  - 漏洞管理：集成漏洞报告，帮助安全团队高效地对发现的问题进行分类和修复
* 运维与监控
  - 环境与发布管理：支持对开发、测试、生产等多环境的管理和部署，并通过功能开关（Feature Flags）实现灰度发布，降低上线风险。
  - 应用性能监控：集成 Prometheus 和 Grafana，提供开箱即用的应用性能监控和日志分析。
* 包与镜像管理
  - 制品库：内置容器镜像仓库和通用制品库，方便管理、存储和分发应用依赖及构建产物
