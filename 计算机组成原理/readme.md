...
* 二进制编码
  ...
  + 字符串的表示，从编码到数字
    - 使用8位的二进制，加上数字和特殊符号组成了ascll码
    ![ascll码](./imgs/ascll码.png)
    - 随着不同国家的人开始使用计算机，ascll码不够用了，于是不同的国家语言创建了字符集和字符编码，字符集就是字符的集合，字符编码就是用二进制表示出来的一个字典
    - 日常说的 Unicode，其实就是一个字符集，包含了 150 种语言的 14 万个不同的字符。可以用 UTF-8、UTF-16，乃至 UTF-32 来进行编码，存储成二进制。
